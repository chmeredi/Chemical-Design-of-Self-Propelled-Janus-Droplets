function output=particlefinder_class2(fname,n,parameters)
...
for k = 1:num_images
    ...
    %Add 20 rows of background on all sides of the image
    avval=mean(mean(A));
    A1=length(A(:,1));
    A2=length(A(1,:));
    Anew=NaN(A1+2*n,A2+2*n);
    Anew(1:n,:)=avval;
    Anew(A1+n+1:A1+2*n,:)=avval;
    Anew(:,1:n)=avval;
    Anew(:,A2+n+1:A2+2*n)=avval;
    Anew(n+1:A1+n,n+1:A2+n)=A;
    A=Anew;
