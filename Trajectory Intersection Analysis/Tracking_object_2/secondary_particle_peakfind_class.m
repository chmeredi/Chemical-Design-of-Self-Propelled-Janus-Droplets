function output=secondary_particle_peakfind_class(A,fname,k,find_parameters,dist_filt_check)

% Unpack information in find_parameters
diameter = find_parameters{1};
lnoise = find_parameters{2};
filtervalue = find_parameters{3};
peakfindvalue = find_parameters{4};
threshold = find_parameters{5};
...
Ai=imgaussfilt(Ai,filtervalue);
pk = pkfnd(Ai,threshold,peakfindvalue);
if ~isempty(pk)
    pk = cntrd(Ai,pk,2*peakfindvalue+1);
end
...
radiplot=zeros(pkl,1)+peakfindvalue/2;
figure
hold on
...
if try_again_check=='n'
    ...
    A = imread(fname, k);
    new_parameters=initial_particle_peakfind_class(A);
    pk=new_parameters{7};
