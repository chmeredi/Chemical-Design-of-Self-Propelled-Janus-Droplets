classdef droplets < particles
    ...
    properties
        contourfile
        speedvals       %values of chaser speed and other droplet speed
    ...
    methods
        ...
        function obj=calc_speed_chaser_ratio(obj)
            %calculates the speed of the chasers with respect to the speed
            %of the other droplets. The number that comes out is a measure
            %for how successful the chase is.
            
            if isempty(obj.tr)
                obj.tr=track(obj);
            end
            
            %smooth the trace of the object
            obj=smooth_trace(obj);
            
            %calculate speed as function of time
            ...
            %Ask user how many predator prey pairs he/she wants to track
            message1='How many droplets that are part of a chase do you want to measure the speed of?';
            n_chase=get_numerical_input(message1);
            
            %Asks user to pick chase particles
            disp('Point out the chasing particles: ')
            picture=obj.im;
            %picture=rgb2gray(picture);
            figure
            colormap('gray'),imagesc(picture)
            ...
            %Ask user how many predator prey pairs he/she wants to track
            message1='How many droplets that are NOT part of a chase do you want to measure the speed of?';
            n_rest=get_numerical_input(message1);
            
            %Asks user to pick other particles
            disp('Point out the non chasing particles: ')
            picture=obj.im;
            %picture=rgb2gray(picture);
            figure
            colormap('gray'),imagesc(picture)
            ...
            %Calculate distances of all particles in first frame to user
            %input points
            distances=NaN(obj.NOP,n_chase);
            tr1=trace(find(trace(:,3)==1),:);
            for p=1:obj.NOP
                trp=tr1(find(tr1(:,4)==p),:);
                for q=1:n_chase
                    distances(p,q)=sqrt((trp(1,1)-x(q))^2+(trp(1,2)-y(q))^2);
                end
            end
            ...
            box on
            %Find minimal distances and corresponding particle IDs
            min_chase=NaN(n_chase,1);
            ID_chase=NaN(n_chase,1);
            speedchasevals=NaN(n_chase,1);
            for q=1:n_chase
                min_chase(q,1)=nanmin(distances(:,q));
                ID_chase(q,1)=find(distances(:,q)==min_chase(q,1));
                chase_speeds=obj.speeds(:,ID_chase(q,1));
                speedchasevals(q,1:2)=[nanmean(chase_speeds(:,1)) nanstd(chase_speeds(:,1))];
                %tr_chase=trace(find(trace(:,4)==ID_chase(q,1)),:);
                plot(chase_speeds(:,1),'r-o');
            end
            average_chasespeed=nanmean(speedchasevals(:,1));
            std_chasespeed=nanstd(speedchasevals(:,1));
            
            
            %Calculate distances of all particles in first frame to user
            %input points
            distances=NaN(obj.NOP,n_rest);
            tr1=trace(find(trace(:,3)==1),:);
            for p=1:obj.NOP
                trp=tr1(find(tr1(:,4)==p),:);
                for q=1:n_rest
                    distances(p,q)=sqrt((trp(1,1)-xr(q))^2+(trp(1,2)-yr(q))^2);
                end
            end
            
            %Find minimal distances and corresponding particle IDs
            min_rest=NaN(n_rest,1);
            ID_rest=NaN(n_rest,1);
            speedrestvals=NaN(n_rest,1);
            for q=1:n_rest
                min_rest(q,1)=nanmin(distances(:,q));
                ID_rest(q,1)=find(distances(:,q)==min_rest(q,1));
                rest_speeds=obj.speeds(:,ID_rest(q,1));
                speedrestvals(q,1:2)=[nanmean(rest_speeds(:,1)) nanstd(rest_speeds(:,1))];
                plot(rest_speeds(:,1),'b-o');
            end
            average_restspeed=nanmean(speedrestvals(:,1));
            std_restspeed=nanstd(speedrestvals(:,1));
            
            disp(['The average chase speed in pix/frame is: ' num2str(average_chasespeed) '.'] )
            disp(['The average background speed in pix/frame is: ' num2str(average_restspeed) '.'] )
            
            obj.speedvals=[average_chasespeed std_chasespeed ; average_restspeed std_restspeed];
        end
    end
end