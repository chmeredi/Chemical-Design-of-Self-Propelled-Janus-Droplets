function output=dropletoptimizer_class(A,sum_parameters,k,pos,dist_filt_check,num_filt_check)
...
elseif ~isempty(find_parameters)
    ...
    r_lower_try=find_parameters{3};
    r_upper_try=find_parameters{4};
    sensitivity_try=find_parameters{5};
...
[centers,radii] = imfindcircles(Ai,[round(r_lower_try),round(r_upper_try)],'Sensitivity',sensitivity_try);
if ~isempty(centers)
    pk=centers(:,1:2);
else
    pk=NaN(1,2);
