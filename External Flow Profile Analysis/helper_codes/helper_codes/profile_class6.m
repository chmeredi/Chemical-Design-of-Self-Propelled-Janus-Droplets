classdef profile_class6
   ...
   methods 
       ...
       function obj=rotate_profile(obj,angle)
            %Rotates flow velocities and grid by a an angle 'angle' in
            %radians
       ...
       function obj=average_profiles_samegrid(listofprofileclasses)
           ...
           for i=1:grid_size(2)
               for j=1:grid_size(1)
                   ...
                   u_new(j,i)=nanmean(Uadd(:,1));%nanmean causes averages to include only few datapoints.
                   v_new(j,i)=nanmean(Vadd(:,1));%use mean if that is not wanted
               end
           end
