classdef particles
    ...
    properties
        ...
        singleparts     % number of single particles each frame; measure for level of aggregation
        allparts        % total number of particles in each individual frame
    ...
    methods
        ...
        function obj=find_pos_confined(obj,n)
            %Creates obj.pos by tracking particles
            %Function is interactive and requires human input
            %initialize indicates whether or not existing parameters are
            %used for particle_finding or a new set is generated
        ...
        function value=disp_hist_xy_sep(obj)
            ...
            %             h=histfit(dx_tot*obj.scale*obj.framerate,10);
            %             h(1).FaceColor='b';
            %             h(1).FaceAlpha=0.5;
            %             h(2).Color='b';
            %             xlabel('Displacement (\mum)','FontSize',16)
            ...
            g=histfit(dy_tot(:,1)*obj.scale*obj.framerate,10);
            g(1).FaceColor='b';
            g(1).FaceAlpha=1;
            g(1).EdgeColor='k';
            g(1).LineWidth=1;
            ...
            value=dy_mean;
            obj.disps={dx_tot,dy_tot};
        end
        function obj=remove_particles_flow(obj,threshold)
            ...
            badparticles=find(meandisp<threshold);
        ...
        function obj=calc_speed_vs_time(obj,particlelist)
            ...
            for q=1:length(particlelist)
                i=particlelist(q);
                plot(tr_i(2:length(tr_i),3)/obj.framerate,spe(:,i)*obj.scale*obj.framerate,'Color',cmap(q,:),'DisplayName',['Particle ' num2str(i)])
            end
            %lgd=legend('show');
            hold off
            ...
            xlabel(' Time (s) ','FontSize',14)
            ylabel(' Speed (\mum/s)  ','FontSize',14)
            plot(tr_i(2:length(tr_i),3)/obj.framerate,spe_mean(:,1)*obj.scale*obj.framerate,'ko','DisplayName','Average');
            hold off
        ...
        function obj=find_neighbours_time_pos(obj,cutoffvalue)
            ...
            if isempty(obj.pos)
                obj=find_pos(obj);
                disp('Found particle positions first because pos was still empty')
            end
            
            input=obj.pos;
            nframes=max(input(:,3));
            %Calculate the number of particles in each frame
            nparts=1;
            for t=1:nframes
                input_t=input(find(input(:,3)==t),:);
                nparts_t=length(input_t);
                if nparts_t>nparts
                    nparts=nparts_t;
                end
            end
            %Create an 2D empty matrix for the number of neighbours
            neighbour=cell(1);
            singleparticles=NaN(nframes,2);
            allparticles=NaN(nframes,2);
            if nparts<2
                obj.dist=distance_all;
                disp('You only have one particle. There are no distances to be calculated');
            else
                for n=1:nframes
                    %Get particle positions in frame n
                    particles=input(find(input(:,3)==n),1:2);
                    
                    %Let user know how far the code is
                    if ~mod(n/100,1)
                        disp(['Working on frame ' num2str(n) ' out of ' num2str(nframes)])
                    end
                    input_t=input(find(input(:,3)==n),:);
                    nparts_t=length(input_t);
                    neighbour_t=NaN(nparts,1);
                    singleparticlecount=0;
                    allpartscount=0;
                    for i=1:nparts_t
                        distance_i=NaN(nparts_t,1);
                        for k=1:nparts_t
                            %Get distance matrix
                            distance_i(k,1)=sqrt((particles(i,1)-particles(k,1))^2+(particles(i,2)-particles(k,2))^2);
                        end%end loop particles k
                        neighbourstest=find(distance_i(:,1)<cutoffvalue);
                        %save number of neighbours to empty matrix
                        if ~isempty(neighbourstest)
                            neighbour_t(i,1)=length(neighbourstest)-1;
                            if neighbour_t(i,1)==0
                                singleparticlecount=singleparticlecount+1;
                            ...
                            neighbour_t(i,1)=NaN;
                        end%end if neighbours
                    end%end loop partcicles i
                    allparticles(n,1)=n;
                    allparticles(n,2)=allpartscount;
                    singleparticles(n,1)=n;
                    singleparticles(n,2)=singleparticlecount;
                    neighbour{n}=neighbour_t;
                end%end loop frames
            end%end if nparts>2
            ...
            obj.singleparts=singleparticles;
            obj.allparts=allparticles;
            
            figure
            hold on
            box on
            plot(singleparticles(:,1)/obj.framerate/1,singleparticles(:,2)./allparticles(:,2),'k.-');
            xlabel(' Time (min) ','FontSize',14)
            ylabel(' Fraction of single particles ','FontSize',14)
            axis([0 240 0 0.2])
            hold off
            ...
            hold on
            box on
            plot(singleparticles(:,1)/obj.framerate/1,singleparticles(:,2),'k.-');
            xlabel(' Time (min) ','FontSize',14)
            ylabel(' Number single particles ','FontSize',14)
            hold off
        ...
        function obj=smooth_trace(obj)
            ...
            for p=1:obj.NOP
                ...
                hold on
                plot(trp(:,3),trp(:,2),'r-')
                trp(:,2)=smooth(trp(:,3),trp(:,2),19);
                plot(trp(:,3),trp(:,2),'b-')
                hold off
                
                obj.tr(find(obj.tr(:,4)==p),:)=trp;
            end
        ...
        function plot_speed_vs_dist(obj,ref_pos)
            %Plots the speed of your particles as function of distance to a
            %reference position. When no reference position is provided,
            %chooses the origin. Provide position as [x y] coordinates.
            ...
            hold on
            box on
            for p=1:obj.NOP %Loop over all particles
                tracks_p=obj.tr(find(obj.tr(:,4)==p),1:2); %Get positions of particle p in time
                speed_p=smooth(obj.speeds(:,p)); %Get speed of particle p in time
                dist_to_ref=sqrt((tracks_p(:,1)-ref_pos(1)).^2+(tracks_p(:,2)-ref_pos(2)).^2); %Calculate distance of particle p with respect to reference point
                plot(dist_to_ref(1:end-1,1)*obj.scale,speed_p*obj.scale*obj.framerate,'o','Color',cmap(p,:)); %Use this to plot speed verus distance to reference position
                %plot(tracks_p(1:end-1,1)*obj.scale,speed_p*obj.scale*obj.framerate,'o','Color',cmap(p,:)); %use this to plot speed versus x coordinate
                %plot(tracks_p(1:end-1,2)*obj.scale,speed_p*obj.scale*obj.framerate,'o','Color',cmap(p,:)); %use this to plot speed versus y coordinate
                %plot3(tracks_p(1:end-1,1)*obj.scale,tracks_p(1:end-1,2)*obj.scale,speed_p*obj.scale*obj.framerate,'o','Color',cmap(p,:)); %Use this to make 3D plot with speed versus x and y coordinate
                xlabel('Distance to reference poin (\um)');
                ylabel('Instantaneous speed (\um/s)');
            end
        end
        function output=output_PIV_mask(obj)
        ...
        function plot_histogram_diffs(obj,n)
            %calculates the diffusion constant for each particle
            %individually and plots them in a histogram
            ...
            for p=1:obj.NOP
                ...
                x=MSD_p(1:10,1);
                y=MSD_p(1:10,2);
                D(1,p)=(x\y)/4*obj.scale^2*obj.framerate;
            end
            ...
            xlabel('Diffusion constant (\um/s)');
            ylabel('Occurence');
            hold off
            
            
        end
        function output=find_fluorescence_values(obj,r)
            %only works if particles has only one frame
            %takes a 16bit image and particle positions. Returns a list of
            %the maximum intensity in a radius r around each particle. Also
            %returns the average background value
            
            if nargin<2
                r=5; %Default value for particle is 11x11 (5 pixels in every direction)
            end
            
            numpars=length(obj.pos(:,1));
            par_vals=NaN(numpars,1);
            image=imread([obj.foldername obj.filename '.tif']);
            imresidual=image;
            
            for p=1:numpars
                x=obj.pos(p,1);
                y=obj.pos(p,2);
                par_vals(p,1)=max(max(image(y-r:y+r,x-r:x+r)));
                imresidual(y-r:y+r,x-r:x+r)=NaN;
            end
            backgroundval=nanmean(nanmean(imresidual));
            disp(backgroundval);
            xvals=[backgroundval,backgroundval+0.001*backgroundval];
            yvals=[0,500];
            
            figure
            hold on
            box on
            h=histogram(par_vals,'FaceColor','b','FaceAlpha',1);
            plot(xvals,yvals,'r')
            xlabel('Raw fluorescence count','FontSize',14)
            ylabel('Number of particles','FontSize',14)
            hold off
